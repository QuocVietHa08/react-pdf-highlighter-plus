// Ported from @anaralabs/lector (lib/dark-mode.ts). Pure, self-contained.
// Hue-preserving dark-mode recolor: flips perceived lightness onto a
// background<->foreground ramp in OKLab while keeping hue/chroma, so red stays
// red and blue links stay blue (unlike a CSS invert + hue-rotate).
 ...
 * Position on the ramp follows gamma-encoded BT.709 luma so saturated accents
 * move the way readers expect (red becomes pink, not cyan; blue links become
 * light blue). Neutrals land exactly on the ramp: white maps to `background`,
 * black to `foreground`, including the palette's tint.
 */
export function createDarkModeColorMap(colors?: DarkModeColors): RenderColorMap {
  ...
  // Exact poles: pure white/black must reproduce the palette strings verbatim
  // (no float round-tripping), so page backgrounds match CSS.
  ...
  // stable function identity per palette, and a wholesale clear would hand a
  // still-mounted consumer a new instance.
